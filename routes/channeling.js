...
router.route('/update/:id').put(async (req, res) => {
    ...
    const AcceptedChanneling = await Channeling.findById(channelingId);
    const appointmentId = AcceptedChanneling.appointmentId;
    const patientId = AcceptedChanneling.patientId;
    const doctorId = AcceptedChanneling.doctorId;
    var approve = true;
    ...
    Channeling.findByIdAndUpdate(channelingId, updateChanneling).then((update) => {
        res.json({ state: true, message: "Channeling accepted.", details: update });
    }).catch((err) => {
...
router.route('/patient/:id').get(async(req, res) => {
    ...
    if(!channelings.length)
        ...
        for(var i=0;i<channelings.length;i++){
            ...
            channels.push({doctorName:doctors.name,date:appointments.date,time:appointments.time,zoomLink:channelings[i].zoomLink});
        }
        res.json({state:true,details:channels});
    }
})
